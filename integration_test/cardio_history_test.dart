...
void main() {
  ...
  testWidgets('offline cardio entry and 10,000 activity history',
    ...
    await tester.enterText(
        find.widgetWithText(TextFormField, 'Distance (km)'), '5',);
    await tester.enterText(
        find.widgetWithText(TextFormField, 'Duration (minutes)'), '29',);
    ...
    await tester.pumpAndSettle();
    for (var i = 0; i < 100 && find.text('5:48 min/km').evaluate().isEmpty; i++) {
    ...
    for (var i = 0; i < 31; i++) {
      ...
      final page = await ActivityHistory(database)
          .query(offset: i * 100)
          .get();
      watch.stop();
